...
const AdminEditProductDetails = ({propData,pid,getAllProducts,onClose}) => {

    const [data,setData]=useState({
        pid,
        productName: propData?.productName,
        brandName:propData?.brandName,
        category:propData?.category,
        productImage:propData?.productImage,
        description:propData?.description,
        price:propData?.price,
        sellingPrice:propData?.sellingPrice
    })
    ...
    const handleUploadSubmit=async(e)=>{
      ...
      const res=await axios.put(`${summaryAPI.updateProductDetail.URL}`,{data},{
        ...
        <div className="fixed bottom-0 top-0 left-0 right-0 w-full h-full z-10 flex items-center justify-center bg-slate-100 bg-opacity-40">
          <div className="mx-auto bg-white shadow-md p-2 w-full h-full rounded-lg max-w-3xl max-h-[80%] text-center overflow-hidden ">
            <div className="flex items-center justify-between mx-2 pb-4">
               <h1 className="text-md font-semibold"> Edit Product Details</h1>
            ...
            <form onSubmit={handleUploadSubmit} className="grid p-4 gap-2 overflow-y-scroll h-full overflow-x-hidden scrollbar mb-10">
                ...
                <div className="p-2 h-40 w-full bg-slate-100 rounded border flex items-center justify-center cursor-pointer">
                    ...
                    <input type="file" id="uploadImageInput"  className=" opacity-0 " onChange={handleUploadImageCloudinary}/>
                    </div>
                ...
                <button className='bg-red-600 text-lg text-white p-1 rounded-sm cursor-pointer  w-fit m-auto px-5 hover:bg-red-800 hover:scale-110 transition-all mb-10'>
